...
server.registerTool(
  "imagegen_jobs_review",
  {
    ...
    description: "Record a per-page visual review across consistency, template invariants, protocol alignment, reference fidelity, text legibility, and artifact quality.",
    inputSchema: {
      jobsPath: z.string().describe("imagegen-jobs.json path."),
      page: z.number().describe("Page number."),
      ...
      templateInvariants: z.enum(["pass", "warn", "fail"]).optional().describe("Whether page-number, footer, and fixed template elements follow the deck-wide contract; logo consistency is soft guidance."),
      protocolAlignment: z.enum(["pass", "warn", "fail"]).optional().describe("Whether the generated image follows the page protocol."),
      referenceFidelity: z.enum(["pass", "warn", "fail"]).optional().describe("Whether referenced assets, figures, tables, numbers, and captions are preserved."),
  ...
  async ({
    ...
    const result = await reviewJobsFile({
      ...
      templateInvariants: templateInvariants || null,
